...
<body>
                    ...
                    <v-card class="overflow-hidden" color="rgb(0, 0, 0, 0.3)" dark>
                        ...
                        <v-form ref="form" lazy-validation v-model="scheduleForm">
                            <v-container>
                                ...
                                <v-btn-toggle v-model="taskType" mandatory disabled>
                                    <v-btn disabled>
                                        Event
                                    </v-btn>
                                    <v-btn @click="removeMap" disabled>
                                        Task/Reminder
                                    </v-btn>
                                    <v-btn @click="removeMap" disabled>
                                        Unavailable
    ...
    <script>
        ...
        function loadEdit(itemid, tasktype, title, date, startTime, endTime, location, description, repeatable) {
            ...
            if (tasktype == "event") {
                FormApp.taskType = 0
                FormApp.titleName = title;
                FormApp.taskID = itemid;
                FormApp.selectLocation = location;
                FormApp.eLocation = location;
                FormApp.eDateInput = date;
                FormApp.estartTimeInput = startTime;
                FormApp.eEndTimeInput = endTime;
                FormApp.eDescription = description;
            } else if (tasktype == "task") {
                FormApp.taskType = 1
                FormApp.titleName = title;
                FormApp.taskID = itemid;
                FormApp.tDateInput = date;
                FormApp.tStartTimeInput = startTime;
                FormApp.tEndTimeInput = endTime;
                FormApp.tDescription = description;
                FormApp.tRepeatable = repeatable;
            } else {
                FormApp.taskType = 2
                FormApp.titleName = title;
                FormApp.taskID = itemid;
                ...
                if (validateTimer(FormApp.tStartTimeInput, FormApp.tEndTimeInput) == false) {
                    ...
                    if (FormApp.taskType == '1') {
                        ...
                        $(document).ready(function() {
                            ...
                            var start_time = FormApp.tStartTimeInput;
                            var title = FormApp.titleName;
                            var unavailable_id = FormApp.taskID
                            ...
                            $.ajax({
                                url: "./include/edit_unavailable.php",
                                type: "POST",
                                data: {
                                    date: date,
                                    description: description,

...
<head>
    ...
    <style>
        ...
        .timer-wrapper .time, .timer-wrapper .sessionText {
            bottom: 20%;
            left: 0;
            position: absolute;
            right: 0;
...
<body>
    ...
    <div id="workApp">
        <v-app style="background: linear-gradient(90deg, #00d2ff 0%, #3a47d5 100%);">
            ...
            <v-main style ="border-radius: 3px;">
            
            <h1>Pomodoro Timer</h1>
            <div class="workStatus"><h4>Now Your Work Status:   </h4><label id = label ><h4>IDLE Time</h4></label></div>
            ...
            <div class="change session"><h4>Working Time Setting:</h4><span>{{ sessionTime | zeroPad }}</span><input v-model="sessionTime" type="range" min="1" max="60" /></div>
            <div class="change break"><h4>Break Time Setting:    </h4><span>  {{ breakTime | zeroPad }}</span><input v-model="breakTime" type="range" min="1" max="60" /></div>
            ...
            loadReq.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    for (i = 0; i < response.length; i++) {
                        ...
                        start = response[i].date + " " + response[i].start_time;
                        end   = response[i].date + " " + response[i].end_time;
                        status =  response[i].title;
                        statusChange(start,end,status);
                    }
                }
            };
